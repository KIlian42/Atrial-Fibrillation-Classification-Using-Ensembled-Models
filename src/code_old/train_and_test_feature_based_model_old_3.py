...
def main():
    ...
    for index, row in enumerate(tqdm(data)):
        if data[index]["id"] not in ids_set:
            if data[index]["label"] == "3" or data[index]["label"] == "4":
                continue
            features = [float(x) for x in data[index]["features"]]
            X.append(features)
            Y.append(data[index]["label"])
    ...
    oversample_strategy = {0: 1513, 1: 1513, 2: 1513}  # Reduce class x to n
    ...
    undersample_strategy = {0: 319, 1: 319, 2: 319}  # Reduce class x to n
    ...
    sns.heatmap(
        cm, annot=True, fmt="d", cmap="Blues", xticklabels=["SR", "AF", "AFL"], yticklabels=["SR", "AF", "AFL"]
    )
    fig = plt.gcf()
    ...
    plt.savefig(
        "ConfusionMatrix_Physionet2021_featureBased_3.png"
    )
    plt.show()
