...
class MyViT(nn.Module):
    def __init__(self, chw, n_patches=1, n_blocks=2, hidden_d=8, n_heads=2, out_d=4):
        # Super constructor
...
if __name__ == '__main__':
    # Load data
    ID, X, Y_labels = load_data()
    
    # Encode labels as integers
    encoder = LabelEncoder()
    encoder.fit(Y_labels)
    Y = encoder.transform(Y_labels)
    # y_original_labels = encoder.inverse_transform(Y)
    ...
    # Create model
    model = MyViT((1, 95, 95), n_patches=1, n_blocks=2, hidden_d=8, n_heads=2, out_d=len(encoder.classes_)).to(device)
    # Load model
    model.load_state_dict(torch.load("src/Models/Trained Models/mymdodel_large"))
